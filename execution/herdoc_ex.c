...
/*   Updated: 2024/09/09 16:13:29 by chourri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handle_child(const char *limiter, int tmp_fd, char **envp)
{
	...
	if (ft_strchr((char *)limiter, '"') || ft_strchr((char *)limiter, '\''))
		flag = 1;
	quoted_limiter = remove_quotes((char *)limiter);
...
//OLD CTRL + C DOESNT WORK AFTER HEREDOC
...
// 	//added
// 	// printf("limiter =>%p\n", limiter);
// 	// printf("quoted_limiter =>%p\n", limiter); //same address as limiter
// 	// free(quoted_limiter);
// 	close(tmp_fd);
// }
