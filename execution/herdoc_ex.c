...
/*   Updated: 2024/10/02 10:19:39 by chourri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//I added what's bellow to read the entire code
...
void handle_here_doc(const char *limiter, t_command *cmd, int file_counter, char **envp)
{
	...
	if (tmp_fd == -1)
		return;
	t_here_doc here_doc_info = {limiter, tmp_fd, temp_filename, cmd, envp}; //not allowed should be explicit initialization
