...
/*   Updated: 2024/09/13 15:12:53 by chourri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_child_error(const char *limiter, t_token *lst)
{
	...
	if (!quoted_limiter)
	{
		close(tmp_fd);  // Close file before returning
		unlink(temp_filename);  // Remove temp file
		return ;
	}
	...
	free(quoted_limiter);
	close(tmp_fd);  // Ensure file is closed before unlinking
